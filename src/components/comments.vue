<template>
  <div class="comment">
    ...
    <template v-else>
      ...
      >
        <p class="title">Excellent comment</p>
        <Comment
    ...
    <empty v-if="!loading && !shouldHotCommentShow && !shouldCommentShow">No comments yet~</empty>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getComment() {
      ...
      // Hot reviews of the playlist need to be obtained by a separate request interface
      if (this.type === PLAYLIST_TYPE && this.currentPage === 1) {
        const { hotComments: exactHotComments = [] } = await getHotComment({
          id: this.id,
          type: 2 // Playlist type
        })
