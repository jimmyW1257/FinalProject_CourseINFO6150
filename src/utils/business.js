...
export function createSong(song) {
    ...
    return {
        ...
        // Album covers must be added if additional requests are required
        albumId,
        // The mv id, if available, will be linked to the mv in the songTable component.
        mvId,
        ...rest
    }
}
...
// Sometimes there is a mvId but the request is 404 so ask for one before jumping.
export async function goMvWithCheck(id) {
    try {
        await getMvDetail(id)
        goMv(id)
    } catch (error) {
        notify("Failed to get mv")
    }
}
